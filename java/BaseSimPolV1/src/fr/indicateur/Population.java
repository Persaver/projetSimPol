...
// Verifiee : oui
	...
	public void ajoutPopulation (int a, int agemin, int agemax){
		int i, col, lig;
		for (i=a; i>0; i--){
			int random = (int)(Math.random() * (agemax-agemin)*12); // permet de mettre aléatoirement la population dans la tranche d'age choisie.
			col = random/12;
			lig = random%12;
	...
	public void retraitPopulation (int a, int agemin, int agemax){
		...
		int col, lig;
		while(a > 0){
			int random = (int)(Math.random() * (agemax-agemin+1)*12); // permet de retirer aléatoirement la population dans la tranche d'age choisie.
			col = random/12;
			lig = random%12;
			//System.out.println("col = " + col + " lig = " + lig);
