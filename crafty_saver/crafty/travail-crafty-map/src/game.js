Game = {
		...
		getData: function(keyObject) {
			var data = {};
				...
				Ajax.getConstructions(function(data){
					console.log(data.succes);
					if(data.succes){
						if(!Game.gameDatas.menuConstruction){
							Game.gameDatas.menuConstruction = {};
						}
						for(var item in data.succes){
							var key = data.succes[item].designation;

							Game.gameDatas.menuConstruction[key] = data.succes[item];
						}


						//Game.gameDatas.menuConstruction=data.succes;
					}
					console.log(data.succes);
					console.log(Game.gameDatas.menuConstruction);
					Game.addMenuConstruction();
				}),
				Ajax.getBackupConstructions(function(data){
					console.log(data.succes);
					if(data.succes){
						if(!Game.gameDatas.mapObjects.dataEntities){
							if(!Game.gameDatas){
								Game.gameDatas = {		
								};
							}
							if(!Game.gameDatas.mapObjects){
								Game.gameDatas.mapObjects = {		
								};
							}
							Game.gameDatas.mapObjects.dataEntities={}
						}
						for(var item in data.succes){
							var key = data.succes[item].construction.id;
							var construction = Game.getConstructionbyId(key);
							if(construction){
								key = construction.designation + "_" + key;
								Game.gameDatas.mapObjects.dataEntities[key] = data.succes[item];
								Game.gameDatas.mapObjects.dataEntities[key].construction = construction;
								Game.gameDatas.mapObjects.dataEntities[key].attr={
										x:data.succes[item].x,
										y:data.succes[item].x,
										h:construction.h,
										w:construction.w
								};
								Game.gameDatas.mapObjects.dataEntities[key].components = construction.designation;
							}
		...
		getConstructionbyId:function(id){
			for(var key in Game.gameDatas.menuConstruction){
				if(Game.gameDatas.menuConstruction[key].id == id){
					return Game.gameDatas.menuConstruction[key];
				}
			}
		},
		...
		addMenuConstruction: function() {
			...
			if (data != undefined) {
				...
				for (var key in data) {
					...
					newCase.id = "menu_" + key.toLowerCase();
